...
BOOST_AUTO_TEST_CASE(Map_Test)
{
	...
	// Load the Map constructor from the MapLibrary DLL
	HINSTANCE hDLL;
	funcMap Map;
	...
	// if the map library DLL was able to load, perform the following
	if (hDLL != NULL) {
		// Load the Map constructor from the DLL.
...
BOOST_AUTO_TEST_CASE(Reduce_Test)
{
	...
	HINSTANCE reduceLibraryHandle;
	funcReduce Reduce;
	const wchar_t* libName = L"ReduceLibrary";

	// Load the library (DLL).
	reduceLibraryHandle = LoadLibraryEx(libName, NULL, NULL);   // Handle to DLL

	// if the DLL was able to be loaded, perform the following.
	if (reduceLibraryHandle != NULL) {
		// Load the Reduce constructor from the DLL.
		Reduce = (funcReduce)GetProcAddress(reduceLibraryHandle, "Reduce");

		// if the Reduce method was not loaded from the DLL, print message to console.
		if (Reduce != NULL) {
			...
			// Free the handle to the DLL.
			FreeLibrary(reduceLibraryHandle);
		...
		{
			cout << "\nReduce method was not loaded correctly from the ReduceLibrary DLL" << endl;
		}
	}
}
...
BOOST_AUTO_TEST_CASE(FullRunFile_Test)
{
	...
	// if the map library handle was able to load, perform the following.
	if (hDLL != NULL) {
		// Load the Map constructor from the DLL.
		Map = (funcMap)GetProcAddress(hDLL, "Map");
