 ...
 * to a JSON object and displayed to the console. The data being
 * queried from the address represents an NFL team's performance 
 * during the year 2020. Alter the last number in the address to
 * query a different NFL team (numbered 1-32).
 * 
 * The NFL team's records are calculated using the private method
 * "calculateRecord" in the SeasonInfo class. The public method 
 * "displayContent" displays the team name, team code, and record
 * (wins-losses-ties) to the console. The method is called on all
 * 32 NFL teams.
 * 
 * Extra Credit: the team codes are numbered alphabetically based 
 * on the respective city's location.
 * 
 */
...
namespace Project1
{
        ...
        static public void Main(String[] stringArgs)
        {
            ...
            for (int i = 0; i < urlList.Count; i++)
            {
                ...
                try
                {
                    ...
                    Console.WriteLine("------ SEASON PERFORMANCE ------");
                    seasonInfoObj.displayContent();
    ...
    public class SeasonInfo
    {
        ...
        private bool contentsCalculated = false;
        ...
        {
            if (!contentsCalculated)
            {
                calculateTeamCode();
                calculateTeamName();
                calculateRecord();
                contentsCalculated = true;
            }
